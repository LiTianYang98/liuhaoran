...
class SchoolsSpider(scrapy.Spider):
    name = 'schools'
    allowed_domains = ['chsi.com.cn']#学信网网址
    start_urls = []
    ...
    def parse(self, response):
        for tr in response.xpath('//tbody/tr[not(@class="noResult")]'):
            try:
                schName: object = tr.xpath('.//a[re:test(@href,"/zsml/querySchAction.do?")]/text()').extract()[0][7:]
    ...
    def parse_major(self, response):
        try:
            ...
            examRange = response.xpath('//tbody[re:test(@class,"zsml-res-items")]')# 获取所有的方向
            for num in range(0,len(examRange)):
                body = examRange[num]
                item['id'] = response.url[-19:] + str(num+1).zfill(3)#id通常为14位
                item['招生单位'] = majorInfo[0].css('td::text')[1].extract()[7:]
                item['院校特性'] = self.__getSchoolFeature(item['招生单位'])[0:3]
                ...
                item['拟招生人数'] = majorInfo[4].css('td::text')[1].extract()[3:5]
